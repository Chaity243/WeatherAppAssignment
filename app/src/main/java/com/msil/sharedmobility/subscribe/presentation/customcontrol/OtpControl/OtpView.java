...
package com.msil.sharedmobility.subscribe.presentation.customcontrol.OtpControl;
...
public class OtpView extends AppCompatEditText {
    ...
    private void drawOtpView(Canvas canvas) {
        ...
        for (int i = 0; i < otpViewItemCount; i++) {
            ...
            drawItemBackground(canvas, itemState, itemFilled);
            canvas.restore();
            if (itemSelected) {
                drawCursor(canvas);
            }
    ...
    private void drawItemBackground(Canvas canvas, int[] backgroundState, boolean itemFilled) {
        if (itemBackground == null) {
            return;
        }

        float delta = (float) lineWidth / 10;
